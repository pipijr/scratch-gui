...
const OUT_PATH = path.resolve('assets');
...
const collectSimple = function (library, dest, debugLabel = 'Item') {
    library.forEach(item => {
        ...
        if (item.baseLayerMD5) {
            // 2.0 library syntax for costumes
            ++md5Count;
            dest.add(item.baseLayerMD5);
        }
        if (item.md5ext) {
            // 3.0 library syntax for costumes
            ++md5Count;
        ...
        if (md5Count < 1) {
            ...
            console.warn(
                `${debugLabel} has multiple MD5 properties:\n${describe(item)}`
            );
        }
    });
...
const fetchAsset = function (md5, callback) {
    ...
    https.get(getOptions, response => {
        if (response.statusCode !== 200) {
            callback(
                new Error(
                    `Request failed: status code ${response.statusCode} for ${urlHuman}`
                )
            );
        ...
        const stream = fs.createWriteStream(path.resolve(OUT_PATH, md5), {
            encoding: 'binary'
        });
